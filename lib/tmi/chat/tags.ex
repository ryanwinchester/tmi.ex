defmodule TMI.Chat.Tags do
  @moduledoc """
  Parse Twitch IRC tag strings.
  """

  alias TMI.Chat.Parsing.EventMapping
  alias TMI.Chat.Parsing.TagMapping
  ...
  defp tag_map({"msg-param-gift-theme" = key, val}) do
    plan =
      case val do
        ...
        theme ->
          Logger.warning("""
          [TMI.Chat.Tags] You found an unsupported gift theme: `#{inspect(theme)}`
  ...
  defp tag_map({"msg-param-category" = key, val}) do
    milestone =
      case val do
        ...
        milestone ->
          Logger.warning("""
          [TMI.Chat.Tags] You found an unsupported milestone: `#{inspect(milestone)}`
          Please report it as an issue at: <https://github.com/ryanwinchester/tmi.ex>
          """)
  ...
  defp tag_map(tag) do
    ...
    Logger.warning("""
    [TMI.Chat.Tags] You found an unsupported tag: `#{inspect(tag)}`
    Please report it as an issue at: <https://github.com/ryanwinchester/tmi.ex>
    """)
