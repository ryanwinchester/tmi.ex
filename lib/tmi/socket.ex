defmodule TMI.Socket do
  ...
  @required_opts ~w[user_id client_id access_token subscriptions]a
  ...
  def handle_frame({:text, msg}, state) do
    case Jason.decode(msg) do
      ...
      _ ->
        Logger.warning("[TMI.Socket] Unhandled message: #{msg}")
        {:ok, state}
    end
  end
  ...
  def handle_frame({type, msg}, state) do
    Logger.debug("[TMI.Socket] unhandled message type: #{inspect(type)}, msg: #{inspect(msg)}")
    {:ok, state}
  end

  @impl WebSockex
  def handle_cast({:send, {type, msg} = frame}, state) do
    Logger.debug("[TMI.Socket] sending #{type} frame with payload: #{msg}")
    {:reply, frame, state}
  end
  ...
  defp handle_message(%{"message_type" => "session_keepalive"}, _payload, _state) do
    Logger.info("[TMI.Socket] keepalive")
  end
  ...
  #     }
  #
  defp handle_message(_metadata, _payload, _state) do
    # TODO
  end
